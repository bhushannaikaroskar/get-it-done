...
export default function TaskPage() {
    ...
    const filterHandler = (event) =>{
        if(!(event.target.value === "All")){
            setFilteredTask([...taskList])
            setFilteredTask(s => [...s].filter((t)=> t.tags.find(tagValue => tagValue === event.target.value)))
        }else{
            setFilteredTask([...taskList])
        }
    }
	...
	return (
		<div className="taskpage">
			<p className="welcome-message">Hey {user.name}, how you doing today?</p>
			...
			<div className="tasks-container">
				<div className="task-list-title">
					<h1>Todo-Items</h1>
					<div><span className="fw-600 font-large">Filters:  </span>
						<select className="filter-tag" name="filters" id="filters" onChange={filterHandler} >
                            <option value="All" selected="selected">All</option>
							{
                                uniqueTags.map((tag)=>{
                                    return <option id={tag} value={tag}>{tag}</option>
                                })
                            }
						</select>
						
					</div>
					<button className="btn btn-float btn-primary" onClick={toggleModal}>
